...
public class UserResource {
	...
	public Resource<User> retrieveUser(@PathVariable int id) {
		...
		// link to all-users
		final ControllerLinkBuilder allUsersLink = ControllerLinkBuilder
				.linkTo(ControllerLinkBuilder.methodOn(this.getClass()).retrieveAllUsers());

		// UserPosts
		final ControllerLinkBuilder userPosts =  ControllerLinkBuilder.linkTo(ControllerLinkBuilder.methodOn(PostResource.class).retrivePosts(id));

		final Resource<User> resource = new Resource<User>(user);
		resource.add(allUsersLink.withRel("all-users"));
		resource.add(userPosts.withRel("user-posts"));
